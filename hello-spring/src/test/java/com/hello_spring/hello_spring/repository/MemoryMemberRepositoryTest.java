...
class MemoryMemberRepositoryTest {
    ...
    /*
    테스트 클래스 실행시, 테스트 메서드의 실행 순서 보장은 안되기 때문에, 테스트 메서드에서 접근할 객체의 데이터가 예상치 못한 값이 저장될수 있다.
    ...
    */
    ...
    public void save() {
        ...
        // Optional 타입일 경우, get 메서드로 강제로 Optional 타입을 까는것은 좋지않지만, 테스트 코드에서는 허용된다.
        Member result = (Member) memberRepository.findMemberById(member.getId()).get();

        // 아니면, 아래처럼 람다 표현식을 사용할수 있다.
//        Optional<Member> result = memberRepository.findMemberById(member.getId());
//
...
//        });
    ...
    public void findByName() {
        ...
        Member result = (Member) memberRepository.findMemberByName(member1.getName()).get();
